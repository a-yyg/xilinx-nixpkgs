...
void DisplayDecodeProfilingResult() {
  if (0 != countWebPDecode) {
    fprintf(stderr,
            "WebPDecode Total Time: %.6f ms, count:%d, average time: %.6f ms\n",
            timeWebPDecode, countWebPDecode, timeWebPDecode / countWebPDecode);
  }
  if (0 != countDecodeInto) {
    fprintf(stderr,
            "  DecodeInto Total Time: %.6f ms, count:%d, average time: %.6f "
            "ms, percentage:%.6f%\n",
  ...
  if (0 != countParseFrame) {
    fprintf(stderr,
            "    ParseFrame Total Time: %.6f ms, count:%d, average time: %.6f "
            "ms, percentage:%.6f%\n",
  ...
  if (0 != countVP8ParseIntraModeRow) {
    fprintf(stderr,
            "      VP8ParseIntraModeRow Total Time: %.6f ms, count:%d, average "
            "time: %.6f ms, percentage:%.6f%\n",
  ...
  if (0 != countVP8DecodeMB) {
    fprintf(stderr,
            "      VP8DecodeMB Total Time: %.6f ms, count:%d, average time: "
            "%.6f ms, percentage:%.6f%\n",
            timeVP8DecodeMB, countVP8DecodeMB,
            timeVP8DecodeMB / countVP8DecodeMB,
            timeVP8DecodeMB * 100 / timeWebPDecode);
  }
  if (0 != countParseResiduals) {
    fprintf(stderr,
            "        ParseResiduals Total Time: %.6f ms, count:%d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeParseResiduals, countParseResiduals,
            timeParseResiduals / countParseResiduals,
            timeParseResiduals * 100 / timeWebPDecode);
  }
  if (0 != countVP8TransformWHT) {
    fprintf(stderr,
            "          VP8TransformWHT Total Time: %.6f ms, count:%d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeVP8TransformWHT, countVP8TransformWHT,
            timeVP8TransformWHT / countVP8TransformWHT,
            timeVP8TransformWHT * 100 / timeWebPDecode);
  }
  if (0 != countGetCoeffs) {
    fprintf(stderr,
            "          GetCoeffs Total Time: %.6f ms, count:%d, average time: "
            "%.6f ms, percentage:%.6f%\n",
  ...
  if (0 != countParseResidualsIf1) {
    fprintf(stderr,
            "          ParseResidualsIf1 Total Time: %.6f ms, count:%d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeParseResidualsIf1, countParseResidualsIf1,
            timeParseResidualsIf1 / countParseResidualsIf1,
            timeParseResidualsIf1 * 100 / timeWebPDecode);
  }
  if (0 != countParseResidualsLoop1) {
    fprintf(stderr,
            "          ParseResidualsLoop1 Total Time: %.6f ms, count:%d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeParseResidualsLoop1, countParseResidualsLoop1,
            timeParseResidualsLoop1 / countParseResidualsLoop1,
            timeParseResidualsLoop1 * 100 / timeWebPDecode);
  }
  if (0 != countParseResidualsLoop2) {
    fprintf(stderr,
            "          ParseResidualsLoop2 Total Time: %.6f ms, count:%d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeParseResidualsLoop2, countParseResidualsLoop2,
            timeParseResidualsLoop2 / countParseResidualsLoop2,
            timeParseResidualsLoop2 * 100 / timeWebPDecode);
  }
  if (0 != countVP8ProcessRow) {
    fprintf(stderr,
            "      VP8ProcessRow Total Time: %.6f ms, count:%d, average time: "
            "%.6f ms, percentage:%.6f%\n",
            timeVP8ProcessRow, countVP8ProcessRow,
            timeVP8ProcessRow / countVP8ProcessRow,
            timeVP8ProcessRow * 100 / timeWebPDecode);
  }
  if (0 != countReconstructRow) {
    fprintf(stderr,
            "        ReconstructRow Total Time: %.6f ms, count:%d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeReconstructRow, countReconstructRow,
            timeReconstructRow / countReconstructRow,
            timeReconstructRow * 100 / timeWebPDecode);
  }
  if (0 != countFinishRow) {
    fprintf(stderr,
            "        FinishRow Total Time: %.6f ms, count:%d, average time: "
            "%.6f ms, percentage:%.6f%\n",
  ...
  //   fprintf(stderr, "  VP8LDecodeImage Total Time: %.6f ms, count:%d, average
  //   time: %.6f ms, percentage:%.6f%\n",
  //           timeVP8LDecodeImage, countVP8LDecodeImage, timeVP8LDecodeImage /
  //           countVP8LDecodeImage, timeVP8LDecodeImage * 100 /
  //           timeWebPDecode);
  // }
  if (0 != countDecodeImageData) {
    fprintf(stderr,
            "    DecodeImageData Total Time: %.6f ms, count:%d, average time: "
            "%.6f ms, percentage:%.6f%\n",
            timeDecodeImageData, countDecodeImageData,
            timeDecodeImageData / countDecodeImageData,
            timeDecodeImageData * 100 / timeWebPDecode);
  }
  if (0 != countProcessRows) {
    fprintf(stderr,
            "      ProcessRows Total Time: %.6f ms, count:%d, average time: "
            "%.6f ms, percentage:%.6f%\n",
            timeProcessRows, countProcessRows,
            timeProcessRows / countProcessRows,
            timeProcessRows * 100 / timeWebPDecode);
  }
  if (0 != countApplyInverseTransforms) {
    fprintf(stderr,
            "        ApplyInverseTransforms Total Time: %.6f ms, count:%d, "
            "average time: %.6f ms, percentage:%.6f%\n",
  ...
  if (0 != countProcessRowsCopy1) {
    fprintf(stderr,
            "          ProcessRowsCopy1 Total Time: %.6f ms, count:%d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeProcessRowsCopy1, countProcessRowsCopy1,
            timeProcessRowsCopy1 / countProcessRowsCopy1,
            timeProcessRowsCopy1 * 100 / timeWebPDecode);
  }
  if (0 != countVP8LAddGreenToBlueAndRed) {
    fprintf(stderr,
            "          VP8LAddGreenToBlueAndRed Total Time: %.6f ms, count:%d, "
            "average time: %.6f ms, "
            "percentage:%.6f%\n",
  ...
  if (0 != countColorSpaceInverseTransform) {
    fprintf(stderr,
            "          ColorSpaceInverseTransform Total Time: %.6f ms, "
            "count:%d, average time: %.6f ms, "
            "percentage:%.6f%\n",
  ...
  if (0 != countPredictorInverseTransform) {
    fprintf(stderr,
            "          PredictorInverseTransform Total Time: %.6f ms, "
            "count:%d, average time: %.6f ms, "
            "percentage:%.6f%\n",
  ...
  if (0 != countProcessRowsCopy2) {
    fprintf(stderr,
            "          ProcessRowsCopy2 Total Time: %.6f ms, count:%d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeProcessRowsCopy2, countProcessRowsCopy2,
            timeProcessRowsCopy2 / countProcessRowsCopy2,
            timeProcessRowsCopy2 * 100 / timeWebPDecode);
  }
  if (0 != countColorIndexInverseTransform) {
    fprintf(stderr,
            "        ColorIndexInverseTransform Total Time: %.6f ms, count:%d, "
            "average time: %.6f ms, "
            "percentage:%.6f%\n",
...
void DisplayEncodeProfilingResult() {
  ...
  if (0 != countWebPEncode) {
    fprintf(stderr,
            "WebPEncode Total Time: %.6f ms, count:%d, average time: %.6f ms\n",
            timeWebPEncode, countWebPEncode, timeWebPEncode / countWebPEncode);
  }
  if (0 != countApplyTransforms) {
    fprintf(stderr, "lossless encode.......\n");
    fprintf(stderr,
            "  ApplyTransforms\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeApplyTransforms, countApplyTransforms,
            timeApplyTransforms / countApplyTransforms,
            timeApplyTransforms * 100 / timeWebPEncode);
  }
  if (0 != timeVP8ApplyNearLossless) {
    fprintf(stderr,
            "    VP8ApplyNearLossless\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeVP8ApplyNearLossless, countVP8ApplyNearLossless,
            timeVP8ApplyNearLossless / countVP8ApplyNearLossless,
            timeVP8ApplyNearLossless * 100 / timeWebPEncode);
  }
  if (0 != timeVP8ApplyNearLosslessOcl) {
    fprintf(stderr,
            "    VP8ApplyNearLosslessOcl\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
  ...
  if (0 != countApplySubtractGreen) {
    fprintf(stderr,
            "    ApplySubtractGreen\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeApplySubtractGreen, countApplySubtractGreen,
            timeApplySubtractGreen / countApplySubtractGreen,
            timeApplySubtractGreen * 100 / timeWebPEncode);
  }
  if (0 != countEncPredict) {
    fprintf(stderr,
            "    ApplyPredictFilter\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
  ...
  if (0 != countVP8LResidualImage) {
    fprintf(stderr,
            "      VP8LResidualImage\t\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeVP8LResidualImage, countVP8LResidualImage,
            timeVP8LResidualImage / countVP8LResidualImage,
            timeVP8LResidualImage * 100 / timeWebPEncode);
  }
  if (0 != countBestPredict) {
    fprintf(stderr,
            "        GetBestPredictorForTile\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, "
            "percentage:%.6f%\n",
            timeBestPredict, countBestPredict,
            timeBestPredict / countBestPredict,
            timeBestPredict * 100 / timeWebPEncode);
  }
  if (0 != countGetPredModesResiduleForTile) {
    fprintf(stderr,
            "        GetPredModesResiduleForTile Total Time: %.6f ms, "
            "count:%6d, average time: %.6f ms, "
            "percentage:%.6f%\n",
  ...
  if (0 != countGetBestPredictorForTile2) {
    fprintf(stderr,
            "        GetBestPredictorForTile2\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, "
            "percentage:%.6f%\n",
  ...
  if (0 != countEncColorFilt) {
    fprintf(stderr,
            "    ApplyCrossColorFilter\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeEncColorFilt, countEncColorFilt,
            timeEncColorFilt / countEncColorFilt,
            timeEncColorFilt * 100 / timeWebPEncode);
  }
  if (0 != countBestColor) {
    fprintf(stderr,
            "      GetBestColorTransformForTileTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, "
            "percentage:%.6f%\n",
            timeBestColor, countBestColor, timeBestColor / countBestColor,
            timeBestColor * 100 / timeWebPEncode);
  }
  if (0 != countGetBackRef) {
    fprintf(stderr,
            "  GetBackwardReferences\t\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
  ...
  if (0 != countBackwardRefLz77) {
    fprintf(stderr,
            "    BackwardReferencesLz77\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeBackwardRefLz77, countBackwardRefLz77,
            timeBackwardRefLz77 / countBackwardRefLz77,
            timeBackwardRefLz77 * 100 / timeWebPEncode);
  }
  if (0 != countBackwardRefRle) {
    fprintf(stderr,
            "    BackwardReferencesRle\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeBackwardRefRle, countBackwardRefRle,
            timeBackwardRefRle / countBackwardRefRle,
            timeBackwardRefRle * 100 / timeWebPEncode);
  }
  if (0 != countGetHistoImg) {
    fprintf(stderr,
            "  VP8LGetHistoImageSymbols\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeGetHistoImg, countGetHistoImg,
            timeGetHistoImg / countGetHistoImg,
            timeGetHistoImg * 100 / timeWebPEncode);
  }
  if (0 != countHistogramCombineStochastic) {
    fprintf(stderr,
            "    HistogramCombineStochastic\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
  ...
  if (0 != countHistogramCombineStochastic_forloop) {
    fprintf(stderr,
            "    HistogramCombineStochastic_forloop\tTotal Time: %.6f ms, "
            "count:%6d, average time: %.6f ms, "
            "percentage:%.6f%\n",
            timeHistogramCombineStochastic_forloop,
            countHistogramCombineStochastic_forloop,
            timeHistogramCombineStochastic_forloop /
                countHistogramCombineStochastic_forloop,
  ...
  if (0 != countHistogramAddEval) {
    fprintf(stderr,
            "      HistogramAddEval\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeHistogramAddEval, countHistogramAddEval,
            timeHistogramAddEval / countHistogramAddEval,
            timeHistogramAddEval * 100 / timeWebPEncode);
  }
  if (0 != countHistogramRemap) {
    fprintf(stderr,
            "    HistogramRemap\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeHistogramRemap, countHistogramRemap,
            timeHistogramRemap / countHistogramRemap,
            timeHistogramRemap * 100 / timeWebPEncode);
  }
  if (0 != countEncode) {
    fprintf(stderr,
            "  Encode\t\t\tTotal Time: %.6f ms, count:%6d, average time: %.6f "
            "ms, percentage:%.6f%\n",
            timeEncode, countEncode, timeEncode / countEncode,
            timeEncode * 100 / timeWebPEncode);
  }
  if (0 != countEncAnalyze) {
    fprintf(stderr, "lossy encode.......\n");
    fprintf(stderr,
            "  VP8EncAnalyze\t\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeEncAnalyze, countEncAnalyze, timeEncAnalyze / countEncAnalyze,
            timeEncAnalyze * 100 / timeWebPEncode);
  }
  if (0 != countEncAnalyzeOcl) {
    fprintf(stderr, "lossy encode.......\n");
    fprintf(stderr,
            "  VP8EncAnalyzeOcl\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeEncAnalyzeOcl, countEncAnalyzeOcl,
            timeEncAnalyzeOcl / countEncAnalyzeOcl,
            timeEncAnalyzeOcl * 100 / timeWebPEncode);
  }
  if (0 != countEncTokenLoop) {
    fprintf(stderr,
            "  VP8EncTokenLoop\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeEncTokenLoop, countEncTokenLoop,
            timeEncTokenLoop / countEncTokenLoop,
            timeEncTokenLoop * 100 / timeWebPEncode);
  }
  if (0 != countEncLoop) {
    fprintf(stderr,
            "  VP8EncLoop\t\t\tTotal Time: %.6f ms, count:%6d, average time: "
            "%.6f ms, percentage:%.6f%\n",
            timeEncLoop, countEncLoop, timeEncLoop / countEncLoop,
            timeEncLoop * 100 / timeWebPEncode);
  }
  if (0 != countEncLoopOcl) {
    fprintf(stderr,
            "  VP8EncLoopOcl\t\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeEncLoopOcl, countEncLoopOcl, timeEncLoopOcl / countEncLoopOcl,
            timeEncLoopOcl * 100 / timeWebPEncode);
  }
  if (0 != countStatLoop) {
    fprintf(stderr,
            "    StatLoop\t\t\tTotal Time: %.6f ms, count:%6d, average time: "
            "%.6f ms, percentage:%.6f%\n",
            timeStatLoop, countStatLoop, timeStatLoop / countStatLoop,
            timeStatLoop * 100 / timeWebPEncode);
  }
  if (0 != countVP8Decimate_2 && 0 != countEncLoop) {
    fprintf(stderr,
            "      VP8Decimate\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeVP8Decimate_2, countVP8Decimate_2,
            timeVP8Decimate_2 / countVP8Decimate_2,
  ...
  if (0 != countRefineUsingDist) {
    fprintf(stderr,
            "        RefineUsingDistortion\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeRefineUsingDist_2, countRefineUsingDist_2,
            timeRefineUsingDist_2 / countRefineUsingDist_2,
            timeRefineUsingDist_2 * 100 / timeWebPEncode);
  }
  if (0 != countStoreFilterSts) {
    fprintf(stderr,
            "    VP8StoreFilterStats\t\t\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeStoreFilterSts, countStoreFilterSts,
            timeStoreFilterSts / countStoreFilterSts,
            timeStoreFilterSts * 100 / timeWebPEncode);
  }
  if (0 != countCodeResiduals) {
    fprintf(stderr,
            "    CodeResiduals\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeCodeResiduals, countCodeResiduals,
            timeCodeResiduals / countCodeResiduals,
  ...
  if (0 != countVP8Decimate) {
    fprintf(stderr,
            "    VP8Decimate\t\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeVP8Decimate, countVP8Decimate,
            timeVP8Decimate / countVP8Decimate,
  ...
  if (0 != countRefineUsingDist) {
    fprintf(stderr,
            "      RefineUsingDistortion\tTotal Time: %.6f ms, count:%6d, "
            "average time: %.6f ms, percentage:%.6f%\n",
            timeRefineUsingDist, countRefineUsingDist,
            timeRefineUsingDist / countRefineUsingDist,
            timeRefineUsingDist * 100 / timeWebPEncode);
  }
  if (0 != countBestIntra16) {
    fprintf(stderr,
            "      PickBestIntra16\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeBestIntra16, countBestIntra16,
            timeBestIntra16 / countBestIntra16,
            timeBestIntra16 * 100 / timeWebPEncode);
  }
  if (0 != countBestIntra4) {
    fprintf(stderr,
            "      PickBestIntra4\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeBestIntra4, countBestIntra4, timeBestIntra4 / countBestIntra4,
            timeBestIntra4 * 100 / timeWebPEncode);
  }
  if (0 != countBestUV) {
    fprintf(stderr,
            "      PickBestUV\t\tTotal Time: %.6f ms, count:%6d, average time: "
            "%.6f ms, percentage:%.6f%\n",
            timeBestUV, countBestUV, timeBestUV / countBestUV,
            timeBestUV * 100 / timeWebPEncode);
  }
  if (0 != countVP8EmitTokens) {
    fprintf(stderr,
            "    VP8EmitTokens\t\tTotal Time: %.6f ms, count:%6d, average "
            "time: %.6f ms, percentage:%.6f%\n",
            timeVP8EmitTokens, countVP8EmitTokens,
            timeVP8EmitTokens / countVP8EmitTokens,
