...
// cache WEBP_BITS of them ahead. The total of (WEBP_BITS + 8) bits must fit
// into a natural register (with type bit_t). To fetch WEBP_BITS bits from
// bitstream we use a type lbit_t.
//
...
//   bit_t = natural register type for storing 'value_' (which is WEBP_BITS+8
//   bits) range_t = register for 'range_' (which is 8bits only)
