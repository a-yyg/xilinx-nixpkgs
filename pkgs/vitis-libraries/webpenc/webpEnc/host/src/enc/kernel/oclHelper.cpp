...
static void getDeviceInfo(oclHardware &hardware) {
  ...
  size_t size = 0;
  cl_int err = clGetDeviceInfo(hardware.mDevice, CL_DEVICE_LOCAL_MEM_SIZE,
                               sizeof(localMemSize), &localMemSize, &size);
  if (err != CL_SUCCESS) {
    std::cout << oclErrorCode(err) << "\n";
    return;
  }
  err = clGetDeviceInfo(hardware.mDevice, CL_DEVICE_MAX_WORK_GROUP_SIZE,
                        sizeof(groupSize), &groupSize, &size);
  if (err != CL_SUCCESS) {
    std::cout << oclErrorCode(err) << "\n";
    return;
  }
  err = clGetDeviceInfo(hardware.mDevice, CL_DEVICE_MAX_COMPUTE_UNITS,
                        sizeof(unitsSize), &unitsSize, &size);
  if (err != CL_SUCCESS) {
    std::cout << oclErrorCode(err) << "\n";
    return;
  }
  err = clGetDeviceInfo(hardware.mDevice, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                        sizeof(maxDimension), &maxDimension, &size);
  if (err != CL_SUCCESS) {
    std::cout << oclErrorCode(err) << "\n";
    return;
  }
  err = clGetDeviceInfo(hardware.mDevice, CL_DEVICE_MAX_WORK_ITEM_SIZES,
                        sizeof(itemsize), itemsize, &size);
  if (err != CL_SUCCESS) {
    std::cout << oclErrorCode(err) << "\n";
    return;
  }
  err = clGetDeviceInfo(hardware.mDevice, CL_DEVICE_MAX_MEM_ALLOC_SIZE,
                        sizeof(allocMemSize), &allocMemSize, &size);
  if (err != CL_SUCCESS) {
    std::cout << oclErrorCode(err) << "\n";
    return;
  }
  err = clGetDeviceInfo(hardware.mDevice, CL_DEVICE_GLOBAL_MEM_SIZE,
                        sizeof(globalMemSize), &globalMemSize, &size);
  if (err != CL_SUCCESS) {
    std::cout << oclErrorCode(err) << "\n";
    return;
  }
  ...
  for (cl_uint i = 0; i < platformCount; i++) {
    ...
    for (idev = 0; idev < deviceCount; idev++) {
      ...
      if (strcmp(deviceName, "xilinx_u200_gen3x16_xdma_base_1") == 0 ||
          ...
          strcmp(deviceName, target_device) == 0) {
    ...
    // cl_context context = clCreateContextFromType(contextData, type, 0, 0,
    // &err);
    ...
    // cl_command_queue queue = clCreateCommandQueue(context, device_id, 0,
    // &err);
...
extern "C" int getOclSoftware(oclSoftware &soft, const oclHardware &hardware) {
  ...
  err = clGetDeviceInfo(hardware.mDevice, CL_DEVICE_TYPE, sizeof(deviceType),
                        &deviceType, 0);
  if (err != CL_SUCCESS) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
    return -1;
