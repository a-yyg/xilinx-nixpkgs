...
void VP8LFreeHistogram(VP8LHistogram *const histo) { WebPSafeFree(histo); }
...
VP8LHistogram *VP8LAllocateHistogram(int cache_bits) {
  ...
  uint8_t *const memory =
      (uint8_t *)WebPSafeMalloc(total_size, sizeof(*memory));
  if (memory == NULL)
    return NULL;
...
VP8LHistogramSet *VP8LAllocateHistogramSet(int size, int cache_bits) {
  ...
  const int histo_size = VP8LGetHistogramSize(cache_bits);
  const size_t total_size = sizeof(*set) + size * (sizeof(*set->histograms) +
...
static int GetCombinedHistogramEntropy(const VP8LHistogram *const a,
  ...
  *cost +=
      VP8LExtraCostCombined(a->literal_ + NUM_LITERAL_CODES,
                            b->literal_ + NUM_LITERAL_CODES, NUM_LENGTH_CODES);
...
static void UpdateHistogramCost(VP8LHistogram *const h) {
  ...
  h->literal_cost_ =
      PopulationCost(h->literal_, num_codes, NULL) +
  ...
  h->bit_cost_ = h->literal_cost_ + h->red_cost_ + h->blue_cost_ + alpha_cost +
                 distance_cost;
...
static int GetHistoBinIndexLowEffort(const VP8LHistogram *const h,
                                     const DominantCostRange *const c) {
  const int bin_id =
...
static int GetHistoBinIndex(const VP8LHistogram *const h,
  ...
  const int bin_id =
      ...
      NUM_PARTITIONS * NUM_PARTITIONS *
          GetBinIdForEntropy(c->literal_min_, c->literal_max_,
                             h->literal_cost_);
...
static void HistogramAnalyzeEntropyBin(VP8LHistogramSet *const image_histo,
  ...
  for (i = 0; i < histo_size; ++i) {
    int num_histos;
    VP8LHistogram *const histo = histograms[i];
    const int16_t bin_id =
        low_effort ? (int16_t)GetHistoBinIndexLowEffort(histo, &cost_range)
...
static VP8LHistogram *
HistogramCombineEntropyBin(VP8LHistogramSet *const image_histo,
                           VP8LHistogram *cur_combo, int16_t *const bin_map,
    ...
    for (n = 2; n <= num_histos; ++n) {
      ...
      if (low_effort) {
        ...
        if (bit_cost_idx2 > 0.) {
          ...
          const double curr_cost_diff = HistogramAddEval(
              histograms[idx1], histograms[idx2], cur_combo, bit_cost_thresh);
...
static int HistoQueueInit(HistoQueue *const histo_queue, const int max_index) {
  ...
  histo_queue->queue =
      WebPSafeMalloc(histo_queue->max_size + 1, sizeof(*histo_queue->queue));
...
static void UpdateQueueFront(HistoQueue *const histo_queue) {
  if (histo_queue->queue[histo_queue->size].cost_diff >= 0)
    return;
...
static VP8LHistogram *
HistogramCombineStochastic(VP8LHistogramSet *const image_histo,
  ...
  image_histo->size = image_histo_size;
  StopProfiling(&stop_watch, &timeHistogramCombineStochastic,
                &countHistogramCombineStochastic);
  return best_combo;
}
...
static HistoWorkerData *HistogramAddThreshAlloc(HistoWorkerData *worker_data,
                                                int num_workers) {
...
static int HistogramAddThreshWorker(HistoWorkerData *const worker_data,
                                    HistoWorkerData *const _worker_data) {
...
static void HistogramAddThreshMt(const VP8LHistogramSet *const orig_histo,
  ...
  WebPWorker *workers =
      (WebPWorker *)WebPSafeMalloc(num_workers, sizeof(*workers));
  if (workers == NULL) {
  ...
  for (n = 0; n < num_workers; ++n) {
    ...
    if (n < num_workers - 1 && !winterface->Reset(worker)) {
      fprintf(stderr,
              "encoder threads for HistogramAddThreshMt creation failed\n");
      return;
    }
  }

  HistoWorkerData *worker_data =
      HistogramAddThreshAlloc(worker_data, num_workers);
  if (NULL == worker_data) {
...
int VP8LGetHistoImageSymbols(int xsize, int ysize,
                             ...
                             int low_effort, int histo_bits, int cache_bits,
                             VP8LHistogramSet *const image_histo,
