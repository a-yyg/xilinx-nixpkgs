...
static int CheckNonOpaque(const uint8_t *alpha, int width, int height,
  ...
  while (height-- > 0) {
    ...
    for (x = 0; x < width * x_step; x += x_step) {
      if (alpha[x] != 0xff)
        return 1; // TODO(skal): check 4/8 bytes at a time.
    }
    alpha += y_step;
  }
  return 0;
...
static WEBP_INLINE float GammaToLinearF(int v) { return kGammaToLinearTabF[v]; }
...
static int UpdateChroma(const fixed_y_t *src1, const fixed_y_t *src2,
...
static void ImportOneRow(const uint8_t *const r_ptr, const uint8_t *const g_ptr,
                         const uint8_t *const b_ptr, int step, int pic_width,
                         fixed_y_t *const dst) {
  int i;
...
static void InterpolateTwoRows(const fixed_y_t *const best_y,
                               ...
                               const fixed_t *const next_uv, int w,
                               fixed_y_t *const out1, fixed_y_t *const out2) {
  int i, k;
...
static int PreprocessARGB(const uint8_t *const r_ptr,
  ...
  if (best_y == NULL || best_uv == NULL || target_y == NULL ||
      target_uv == NULL || best_rgb_y == NULL || best_rgb_uv == NULL ||
      tmp_buffer == NULL) {
  ...
  for (j = 0; j < picture->height; j += 2) {
    ...
    ImportOneRow(r_ptr + off1, g_ptr + off1, b_ptr + off1, step, picture->width,
                 src1);
    if (!is_last_row) {
      ImportOneRow(r_ptr + off2, g_ptr + off2, b_ptr + off2, step,
                   picture->width, src2);
    } else {
  ...
  for (iter = 0; iter < kNumIterations; ++iter) {
    ...
    for (j = 0; j < h; j += 2) {
      ...
      fixed_y_t *const src2 = tmp_buffer + 3 * w;
      {
        ...
        InterpolateTwoRows(best_y + j * w, prev_uv, cur_uv, next_uv, w, src1,
                           src2);
        prev_uv = cur_uv;
        cur_uv = next_uv;
      }
...
AccumulateRGBA(const uint8_t *const r_ptr, const uint8_t *const g_ptr,
  ...
  if (use_iterative_conversion) {
    ...
    for (y = 0; y < (height >> 1); ++y) {
      ...
      if (use_dsp) {
        ...
        ConvertRowToY(r_ptr + off1, g_ptr + off1, b_ptr + off1, step, dst_y,
                      width, rg);
      ...
      if (has_alpha) {
        rows_have_alpha &= !WebPExtractAlpha(a_ptr + off1, rgb_stride, width, 2,
                                             dst_a, picture->a_stride);
        dst_a += 2 * picture->a_stride;
      }
      // Collect averaged R/G/B(/A)
      if (!rows_have_alpha) {
        AccumulateRGB(r_ptr + off1, g_ptr + off1, b_ptr + off1, step,
    ...
    if (height & 1) { // extra last row
      ...
      if (use_dsp) {
        ...
        ConvertRowToY(r_ptr + off, g_ptr + off, b_ptr + off, step, dst_y, width,
                      rg);
      }
      if (row_has_alpha) {
      ...
      if (!row_has_alpha) {
        ...
        AccumulateRGB(r_ptr + off, g_ptr + off, b_ptr + off, step,
...
static int Import(WebPPicture *const picture, const uint8_t *const rgb,
  ...
  if (import_alpha) {
    ...
    for (y = 0; y < height; ++y) {
      uint32_t *const dst = &picture->argb[y * picture->argb_stride];
      const int offset = y * rgb_stride;
      VP8PackRGB(r_ptr + offset, g_ptr + offset, b_ptr + offset, width, step,
                 dst);
    }
  }
  return 1;
...
int WebPPictureImportBGR(WebPPicture *picture, const uint8_t *rgb,
                         int rgb_stride) {
...
int WebPPictureImportBGRA(WebPPicture *picture, const uint8_t *rgba,
                          int rgba_stride) {
  return (picture != NULL) ? Import(picture, rgba, rgba_stride, 4, 1, 1) : 0;
}

int WebPPictureImportRGBX(WebPPicture *picture, const uint8_t *rgba,
                          int rgba_stride) {
  return (picture != NULL) ? Import(picture, rgba, rgba_stride, 4, 0, 0) : 0;
}

int WebPPictureImportBGRX(WebPPicture *picture, const uint8_t *rgba,
                          int rgba_stride) {
