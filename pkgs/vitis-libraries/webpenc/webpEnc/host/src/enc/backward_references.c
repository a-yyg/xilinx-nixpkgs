...
static WEBP_INLINE int FindMatchLength(const uint32_t *const array1,
  ...
  if (memcmp(array1, array2, best_len_match * sizeof(*array1)))
    return 0;
...
static void HashChainInsert(VP8LHashChain *const p, const uint32_t *const argb,
                            int pos) {
...
static int HashChainFindCopy(const VP8LHashChain *const p, int base_position,
                             const uint32_t *const argb, int max_len,
                             int window_size, int iter_max,
                             int *const distance_ptr, int *const length_ptr) {
  const uint32_t *const argb_start = argb + base_position;
  int iter = iter_max;
  int best_length = 0;
  int best_distance = 0;
...
static int BackwardReferencesLz77(int xsize, int ysize,
  ...
  for (i = 0; i < pix_count - 2;) {
    ...
    int len = 0;
    const int max_len = MaxFindCopyLength(pix_count - i);
    HashChainFindCopy(hash_chain, i, argb, max_len, window_size, iter_max,
                      &offset, &len);
    if (len > MIN_LENGTH || (len == MIN_LENGTH && offset <= 512)) {
      ...
      if ((len < (max_len >> 2)) && !low_effort) {
        ...
        HashChainFindCopy(hash_chain, i + 1, argb, max_len - 1, window_size,
                          iter_max, &offset2, &len2);
        if (len2 > len + 1) {
          AddSingleLiteral(argb[i], use_color_cache, &hashers, refs);
...
static int CostModelBuild(CostModel *const m, int cache_bits,
  ...
  ConvertPopulationCountTableToBitEstimates(
      ...
      m->literal_);
  ConvertPopulationCountTableToBitEstimates(VALUES_IN_BYTE, histo->red_,
                                            m->red_);
  ConvertPopulationCountTableToBitEstimates(VALUES_IN_BYTE, histo->blue_,
                                            m->blue_);
  ConvertPopulationCountTableToBitEstimates(VALUES_IN_BYTE, histo->alpha_,
                                            m->alpha_);
  ConvertPopulationCountTableToBitEstimates(NUM_DISTANCE_CODES,
                                            histo->distance_, m->distance_);
  ok = 1;
...
static WEBP_INLINE double GetLiteralCost(const CostModel *const m, uint32_t v) {
  return m->alpha_[v >> 24] + m->red_[(v >> 16) & 0xff] +
  ...
  for (i = 1; i < pix_count - 1; ++i) {
    ...
    if (len >= MIN_LENGTH) {
      ...
      if (len != MIN_LENGTH) {
        ...
        cost_total = prev_cost + GetDistanceCost(cost_model, code_min_length) +
...
static int BackwardReferencesHashChainFollowChosenPath(
    int xsize, int ysize, const uint32_t *const argb, int quality,
    int cache_bits, const uint16_t *const chosen_path, int chosen_path_size,
    VP8LHashChain *const hash_chain, VP8LBackwardRefs *const refs) {
  const int pix_count = xsize * ysize;
  const int use_color_cache = (cache_bits > 0);
  int ix;
  int i = 0;
...
static int BackwardReferencesTraceBackwards(int xsize, int ysize,
  ...
  if (!BackwardReferencesHashChainDistanceOnly(xsize, ysize, argb, quality,
                                               cache_bits, hash_chain, refs,
                                               dist_array)) {
    goto Error;
  }
  TraceBackwards(dist_array, dist_array_size, &chosen_path, &chosen_path_size);
  if (!BackwardReferencesHashChainFollowChosenPath(
          xsize, ysize, argb, quality, cache_bits, chosen_path,
  ...
  while (eval_low || eval_high) {
    ...
    if (entropy_high < entropy_low) {
      ...
      *best_cache_bits = cache_bits_high;
      ...
      if (cache_bits_low != prev_cache_bits_low)
        eval_low = 1;
    } else {
      *best_cache_bits = cache_bits_low;
      cache_bits_high = (cache_bits_low + cache_bits_high) / 2;
      if (cache_bits_high != cache_bits_low)
        eval_high = 1;
    }
  }
  *lz77_computed = 1;
...
static VP8LBackwardRefs *GetBackwardReferences(int width, int height,
                                               const uint32_t *const argb,
                                               int quality,
