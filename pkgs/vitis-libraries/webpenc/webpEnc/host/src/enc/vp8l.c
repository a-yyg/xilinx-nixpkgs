...
static int AnalyzeAndCreatePalette(const WebPPicture *const pic, int low_effort,
...
static void AddSingle(uint32_t p, uint32_t *a, uint32_t *r, uint32_t *g,
                      uint32_t *b) {
  ++a[p >> 24];
  ++r[(p >> 16) & 0xff];
  ++g[(p >> 8) & 0xff];
...
static int AnalyzeEntropy(const uint32_t *argb, int width, int height,
                          int argb_stride, int use_palette,
...
static int AnalyzeAndInit(VP8LEncoder *const enc) {
  ...
  enc->use_palette_ = AnalyzeAndCreatePalette(pic, low_effort, enc->palette_,
                                              &enc->palette_size_);

  // TODO(jyrki): replace the decision to be based on an actual estimate
  // of entropy, or even spatial variance of entropy.
  enc->histo_bits_ =
      GetHistoBits(method, enc->use_palette_, pic->width, pic->height);
...
static int
GetHuffBitLengthsAndCodes(const VP8LHistogramSet *const histogram_image,
...
static void
StoreHuffmanTreeOfHuffmanTreeToBitMask(VP8LBitWriter *const bw,
...
static void
ClearHuffmanTreeIfOnlyOneSymbol(HuffmanTreeCode *const huffman_code) {
  int k;
  int count = 0;
...
WriteHuffmanCodeWithExtraBits(VP8LBitWriter *const bw,
                              const HuffmanTreeCode *const code, int code_index,
                              int bits, int n_bits) {
  const int depth = code->code_lengths[code_index];
  const int symbol = code->codes[code_index];
...
StoreImageToBitMask(VP8LBitWriter *const bw, int width, int histo_bits,
...
static WebPEncodingError EncodeImageInternal(
    ...
    VP8LHashChain *const hash_chain, VP8LBackwardRefs refs_array[2], int width,
    int height, int quality, int low_effort, int *cache_bits,
    int histogram_bits, size_t init_byte_position, int *const hdr_size,
  ...
  *cache_bits = MAX_COLOR_CACHE_BITS;
  ...
  // Calculate backward references from ARGB image.
  best_refs =
      VP8LGetBackwardReferences(width, height, argb, quality, low_effort,
  ...
  huffman_codes =
      (HuffmanTreeCode *)WebPSafeCalloc(bit_array_size, sizeof(*huffman_codes));
  ...
  *hdr_size = (int)(VP8LBitWriterNumBytes(bw) - init_byte_position);
  // Store actual literals.
  err = StoreImageToBitMask(bw, width, histogram_bits, &refs, histogram_symbols,
                            huffman_codes);
  *data_size =
  ...
  if (enc->argb_ == NULL) {
    ...
    const int transform_data_size =
        (enc->use_predict_ || enc->use_cross_color_)
            ...
            : 0;
    const uint64_t total_size = image_size + WEBP_ALIGN_CST +
...
static WebPEncodingError MakeInputImageCopy(VP8LEncoder *const enc) {
  ...
  if (err != VP8_ENC_OK)
    return err;
  for (y = 0; y < height; ++y) {
    memcpy(enc->argb_ + y * width, picture->argb + y * picture->argb_stride,
...
static WebPEncodingError ApplyPalette(const uint32_t *src, uint32_t src_stride,
  ...
  if (use_LUT) {
    ...
    int last_idx = 0;
    for (y = 0; y < height; ++y) {
      MapToPalette(palette, palette_size, &last_pix, &last_idx, src, tmp_row,
                   width);
      VP8LBundleColorMap(tmp_row, width, xbits, dst);
      src += src_stride;
      dst += dst_stride;
...
static WebPEncodingError MapImageFromPalette(VP8LEncoder *const enc,
  ...
  err = ApplyPalette(src, src_stride, enc->argb_, enc->current_width_, palette,
                     palette_size, width, height, xbits);
  return err;
}
...
static WebPEncodingError
EncodeDeltaPalettePredictorImage(VP8LBitWriter *const bw,
...
WebPEncodingError VP8LEncodeStream(const WebPConfig *const config,
  ...
  if (!use_delta_palettization) {
    ...
    if (enc->use_cross_color_) {
      err =
          ApplyCrossColorFilter(enc, enc->current_width_, height, quality, bw);
      if (err != VP8_ENC_OK)
        goto Error;
    }
  }
