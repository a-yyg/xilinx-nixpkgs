...
static void GetValAndDistance(int a, int initial, int bits, int *const val,
                              int *const distance) {
...
int VP8ApplyNearLossless(int xsize, int ysize, uint32_t *argb, int quality) {
  ...
  if (copy_buffer == NULL) {
    StopProfiling(&stop_watch, &timeVP8ApplyNearLossless,
                  &countVP8ApplyNearLossless);
    return 0;
  }
...
int VP8ApplyNearLosslessOcl(int xsize, int ysize, uint32_t *argb, int quality) {
  ...
  // For small icon images, don't attempt to apply near-lossless compression.
  if (xsize < MIN_DIM_FOR_NEAR_LOSSLESS && ysize < MIN_DIM_FOR_NEAR_LOSSLESS) {
    StopProfiling(&stop_watch, &timeVP8ApplyNearLosslessOcl,
                  &countVP8ApplyNearLosslessOcl);
    return 1;
  }
  ...
  if (xsize > IMAGE_4K) {
    ...
    global_width = RoundUp(xsize - VECTOR_WIDTH_PADDING,
                           VECTOR_GRX_SIZE_4K * VECTOR_LENGTH);
    global_height = RoundUp(ysize - PADDING_SIZE, VECTOR_GRY_SIZE_4K);
  } else {
    device_width = RoundUp(xsize, VECTOR_GRX_SIZE * VECTOR_LENGTH);
    global_width =
        RoundUp(xsize - VECTOR_WIDTH_PADDING, VECTOR_GRX_SIZE * VECTOR_LENGTH);
  ...
  err = clEnqueueWriteBuffer(hardware.mQueue, nearpara.input_argb, CL_TRUE, 0,
                             argb_size, argb, 0, NULL, NULL);
  if (CL_SUCCESS != err) {
  ...
  err = clEnqueueWriteBuffer(hardware.mQueue, nearpara.output_argb, CL_TRUE, 0,
                             argb_size, argb, 0, NULL, NULL);
  if (CL_SUCCESS != err) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
    status = -1;
  ...
  for (int k = limit_bits; k != 0; --k) {
    ...
    err = clSetKernelArg(nearlossless.mKernel, 7, sizeof(int),
                         &(nearpara.limitbits));
    if (err != CL_SUCCESS) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      status = -1;
    ...
    if (k % 2 == limit_bits % 2) {
      err = clSetKernelArg(nearlossless.mKernel, 0, sizeof(cl_mem),
                           &(nearpara.input_argb));
      if (err != CL_SUCCESS) {
        fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
        status = -1;
      ...
      err = clSetKernelArg(nearlossless.mKernel, 1, sizeof(cl_mem),
  ...
  if (limit_bits % 2 == 1) {
    err = clEnqueueReadBuffer(hardware.mQueue, nearpara.output_argb, CL_TRUE, 0,
                              argb_size, argb, 0, NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      status = -1;
    ...
    err = clEnqueueReadBuffer(hardware.mQueue, nearpara.input_argb, CL_TRUE, 0,
                              argb_size, argb, 0, NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      status = -1;
