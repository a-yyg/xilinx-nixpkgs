...
static int RecordTokens(VP8EncIterator *const it, const VP8ModeScore *const rd,
  ...
  if (it->mb_->type_ == 1) { // i16x16
    ...
    VP8SetResidualCoeffs(rd->y_dc_levels, &res);
    it->top_nz_[8] = it->left_nz_[8] =
        VP8RecordCoeffTokens(ctx, 1, res.first, res.last, res.coeffs, tokens);
    VP8RecordCoeffs(ctx, &res);
    VP8InitResidual(1, 0, enc, &res);
  } else {
    VP8InitResidual(0, 3, enc, &res);
  ...
  for (y = 0; y < 4; ++y) {
    for (x = 0; x < 4; ++x) {
      const int ctx = it->top_nz_[x] + it->left_nz_[y];
      VP8SetResidualCoeffs(rd->y_ac_levels[x + y * 4], &res);
      it->top_nz_[x] = it->left_nz_[y] = VP8RecordCoeffTokens(
          ctx, res.coeff_type, res.first, res.last, res.coeffs, tokens);
      VP8RecordCoeffs(ctx, &res);
    }
  }
  ...
  for (ch = 0; ch <= 2; ch += 2) {
    for (y = 0; y < 2; ++y) {
      for (x = 0; x < 2; ++x) {
        ...
        VP8SetResidualCoeffs(rd->uv_levels[ch * 2 + x + y * 2], &res);
        it->top_nz_[4 + ch + x] = it->left_nz_[4 + ch + y] =
            VP8RecordCoeffTokens(ctx, 2, res.first, res.last, res.coeffs,
                                 tokens);
        VP8RecordCoeffs(ctx, &res);
      }
    }
...
int VP8EncTokenLoop(VP8Encoder *const enc) {
  ...
  while (ok && num_pass_left-- > 0) {
    ...
    if (stats.do_size_search) {
      ...
      size +=
          VP8EstimateTokenSize(&enc->tokens_, (const uint8_t *)proba->coeffs_);
...
int VP8EncTokenLoopAsyncHost2Device(const int Numbatch, VP8Encoder **enc,
                                    AllPicInfo *picinfo, const int buf,
  ...
  for (int i = 0; i < Numbatch; i++) {
    ...
    memcpy(encloopparaAsync[buf].inputcpu + offset_info, &picinfo[i],
           sizeof(AllPicInfo));
    offset_info += Get_Busoffset_info_32bits() * sizeof(uint32_t);
  }

  err = clEnqueueMigrateMemObjects(hardware.mQueue, 1,
                                   &(encloopparaAsync[buf].input), 0,
  ...
  err =
      clEnqueueMigrateMemObjects(hardware.mQueue, 1, &(encloopparaAsync[buf].y),
                                 0, num_wait_event, wait_event, &event[1]);
  if (CL_SUCCESS != err) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
  }
  ...
  err =
      clEnqueueMigrateMemObjects(hardware.mQueue, 1, &(encloopparaAsync[buf].u),
                                 0, num_wait_event, wait_event, &event[2]);
  if (CL_SUCCESS != err) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
  }
  ...
  err =
      clEnqueueMigrateMemObjects(hardware.mQueue, 1, &(encloopparaAsync[buf].v),
                                 0, num_wait_event, wait_event, &event[3]);
  if (CL_SUCCESS != err) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
  }
  ...
  err = clEnqueueMigrateMemObjects(
      hardware.mQueue, 1, &(encloopparaAsync[buf].output),
...
int VP8EncTokenLoopAsyncDevice2Host(const int buf, cl_uint num_wait_event,
  ...
  err = clEnqueueMigrateMemObjects(
      hardware.mQueue, 1, &(encloopparaAsync[buf].output_prob),
  ...
  // ret==========================================================//
  err = clEnqueueMigrateMemObjects(
      hardware.mQueue, 1, &(encloopparaAsync[buf].output_ret),
  ...
  // pred=========================================================//
  err = clEnqueueMigrateMemObjects(
      hardware.mQueue, 1, &(encloopparaAsync[buf].output_pred),
  ...
  // bw=============================================================//
  err = clEnqueueMigrateMemObjects(
      hardware.mQueue, 1, &(encloopparaAsync[buf].output_bw),
      CL_MIGRATE_MEM_OBJECT_HOST, num_wait_event, wait_event, &event[3]);
  if (CL_SUCCESS != err) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
    ok = 0;
  }
...
int VP8EncTokenLoopAsyncPredKernel(const int buf, cl_uint num_wait_event,
  ...
  int arg = 0;
  int status;
  err = clSetKernelArg(encloop.mKernelPred[buf], arg++, sizeof(cl_mem),
                       &(encloopparaAsync[buf].input));
  if (err != CL_SUCCESS) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
    status = -1;
  }
  ...
  err = clSetKernelArg(encloop.mKernelPred[buf], arg++, sizeof(cl_mem),
                       &(encloopparaAsync[buf].output_prob));
  if (err != CL_SUCCESS) {
  ...
  err = clEnqueueNDRangeKernel(hardware.mQueue, encloop.mKernelPred[buf], 1,
                               NULL, globalSize, localSize, num_wait_event,
                               wait_event, &event[0]);
  if (err != CL_SUCCESS) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
    ok = 0;
...
int VP8EncTokenLoopAsyncACKernel(const int buf, cl_uint num_wait_event,
  ...
  // 1)
  err = clSetKernelArg(encloop.mKernelAC[buf], arg++, sizeof(cl_mem),
                       &(encloopparaAsync[buf].output));
  if (err != CL_SUCCESS) {
  ...
  // 3)
  err = clSetKernelArg(encloop.mKernelAC[buf], arg++, sizeof(cl_mem),
                       &(encloopparaAsync[buf].output_bw));
  if (err != CL_SUCCESS) {
  ...
  // 4)
  err = clSetKernelArg(encloop.mKernelAC[buf], arg++, sizeof(cl_mem),
                       &(encloopparaAsync[buf].output_ret));
  if (err != CL_SUCCESS) {
  ...
  // 5)
  err = clSetKernelArg(encloop.mKernelAC[buf], arg++, sizeof(cl_mem),
                       &(encloopparaAsync[buf].output_pred));
  if (err != CL_SUCCESS) {
  ...
  err = clEnqueueNDRangeKernel(hardware.mQueue, encloop.mKernelAC[buf], 1, NULL,
                               globalSize, localSize, num_wait_event,
                               wait_event, &event[0]);
  if (err != CL_SUCCESS) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
    ok = 0;
...
void VP8BitWriterCpy_hls(VP8BitWriter *const bw_des, uint8_t *const pout_prob,
                         uint8_t *const pout_bw) {
...
int VP8EncTokenLoopAsyncAfterAC(
  ...
  VP8MBInfo *p_mb_info =
      enc->mb_info_;              // contextual macroblock infos (mb_w_ + 1)
  ...
  for (int y_mb = 0; y_mb < enc->mb_h_; y_mb++) {
    ...
    for (int x_mb = 0; x_mb < enc->mb_w_; x_mb++) {
      ...
      for (int y = 0; y < 4; y++) {
        for (int x = 0; x < 4; x += 2) {
          uint8_t tmp = *p_pred++;
          p_preds[x + enc->preds_w_ * y] =
              tmp & 15; // pt[400+y*4+x];//tmp&15;//pt[400+y*4+x];//
          p_preds[x + 1 + enc->preds_w_ * y] =
              (tmp >> 4) & 15; // pt[400+y*4+x+1];//;(tmp>>4)&15;
        }
      }
      p_preds += 4;
      p_mb_info += 1;
