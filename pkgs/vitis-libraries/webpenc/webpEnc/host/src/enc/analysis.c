...
static int MBAnalyzeBestIntra16Mode(VP8EncIterator *const it) {
  ...
  for (mode = 0; mode < max_mode; ++mode) {
    ...
    VP8CollectHistogram(it->yuv_in_ + Y_OFF_ENC,
                        it->yuv_p_ + VP8I16ModeOffsets[mode], 0, 16, &histo);
...
static int MBAnalyzeBestIntra4Mode(VP8EncIterator *const it, int best_alpha) {
  ...
  do {
    ...
    for (mode = 0; mode < max_mode; ++mode) {
      ...
      VP8CollectHistogram(src, it->yuv_p_ + VP8I4ModeOffsets[mode], 0, 1,
                          &histos[cur_histo]);
...
static int MBAnalyzeBestUVMode(VP8EncIterator *const it) {
  ...
  for (mode = 0; mode < max_mode; ++mode) {
    ...
    VP8CollectHistogram(it->yuv_in_ + U_OFF_ENC,
                        it->yuv_p_ + VP8UVModeOffsets[mode], 16, 16 + 4 + 4,
                        &histo);
    alpha = GetAlpha(&histo);
    if (IS_BETTER_ALPHA(alpha, best_alpha)) {
      best_alpha = alpha;
  ...
  if (do_segments) {
    ...
    if (do_mt) {
      ...
      for (i = 0; i < mt_num - 1; i++) {
        if (more_row > 0) {
          InitSegmentJob(enc, &side_job[i], last_end_row,
                         last_end_row + split_row + 1);
          more_row -= 1;
  ...
  if (do_segments) {
    ...
    if (expand_yheight > height) {
      for (i = 0; i < expand_yheight - height; i++) {
        memcpy(expand_y + i * width, enc->pic_->y + width * (height - 1),
               width);
      }
    }
    ...
    if (expand_uvheight > uv_height) {
      for (i = 0; i < expand_uvheight - uv_height; i++) {
        memcpy(expand_u + i * uv_width,
               enc->pic_->u + uv_width * (uv_height - 1), uv_width);
        memcpy(expand_v + i * uv_width,
               enc->pic_->v + uv_width * (uv_height - 1), uv_width);
      }
    }
    ...
    err = clSetKernelArg(analyze.mKernel, 9, sizeof(cl_int),
                         &(analyzepara.method));
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(hardware.mQueue, analyzepara.y, CL_TRUE, 0,
                               y_size, enc->pic_->y, 0, NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      ok = 0;
      goto Err;
    ...
    if (expand_yheight > height) {
      err =
          clEnqueueWriteBuffer(hardware.mQueue, analyzepara.y, CL_TRUE, y_size,
                               expand_y_size, expand_y, 0, NULL, NULL);
      if (CL_SUCCESS != err) {
        fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
        ok = 0;
        goto Err;
    ...
    err = clEnqueueWriteBuffer(hardware.mQueue, analyzepara.u, CL_TRUE, 0,
                               u_size, enc->pic_->u, 0, NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      ok = 0;
      goto Err;
    ...
    if (expand_uvheight > uv_height) {
      err =
          clEnqueueWriteBuffer(hardware.mQueue, analyzepara.u, CL_TRUE, u_size,
                               expand_uv_size, expand_u, 0, NULL, NULL);
      if (CL_SUCCESS != err) {
        fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
        ok = 0;
        goto Err;
    ...
    err = clEnqueueWriteBuffer(hardware.mQueue, analyzepara.v, CL_TRUE, 0,
                               v_size, enc->pic_->v, 0, NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      ok = 0;
      goto Err;
    ...
    if (expand_uvheight > uv_height) {
      err =
          clEnqueueWriteBuffer(hardware.mQueue, analyzepara.v, CL_TRUE, v_size,
                               expand_uv_size, expand_v, 0, NULL, NULL);
      if (CL_SUCCESS != err) {
        fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
        ok = 0;
        goto Err;
    ...
    err = clEnqueueWriteBuffer(hardware.mQueue, analyzepara.output_alpha,
                               CL_TRUE, 0, mb_h * sizeof(int), &output_alpha, 0,
                               NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      ok = 0;
    ...
    err = clEnqueueWriteBuffer(hardware.mQueue, analyzepara.output_uvalpha,
                               CL_TRUE, 0, mb_h * sizeof(int), &output_uvalpha,
                               0, NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      ok = 0;
    ...
    err = clEnqueueWriteBuffer(hardware.mQueue, analyzepara.alphas, CL_TRUE, 0,
                               mb_h * (MAX_ALPHA + 1) * sizeof(int), alphas, 0,
                               NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      ok = 0;
    ...
    err = clEnqueueReadBuffer(hardware.mQueue, analyzepara.output_alpha,
                              CL_TRUE, 0, mb_h * sizeof(int), &output_alpha, 0,
                              NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
    ...
    err = clEnqueueReadBuffer(hardware.mQueue, analyzepara.output_uvalpha,
                              CL_TRUE, 0, mb_h * sizeof(int), &output_uvalpha,
                              0, NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
    ...
    err = clEnqueueReadBuffer(hardware.mQueue, analyzepara.mb_info, CL_TRUE, 0,
                              3 * mb_size, mb_info, 0, NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      ok = 0;
      goto Err;
    ...
    err = clEnqueueReadBuffer(hardware.mQueue, analyzepara.preds, CL_TRUE, 0,
                              preds_size, enc->preds_, 0, NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
      ok = 0;
      goto Err;
    ...
    err = clEnqueueReadBuffer(hardware.mQueue, analyzepara.alphas, CL_TRUE, 0,
                              mb_h * (MAX_ALPHA + 1) * sizeof(int), alphas, 0,
                              NULL, NULL);
    if (CL_SUCCESS != err) {
      fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(err));
