...
const int VP8I4ModeOffsets[NUM_BMODES] = {I4DC4, I4TM4, I4VE4, I4HE4, I4RD4,
...
static const uint8_t VP8TopLeftI4ttt[16] = {17, 21, 25, 29, 13, 17, 21, 25,
...
static int TrellisQuantizeBlock(const VP8Encoder *const enc, int16_t in[16],
                                int16_t out[16], int ctx0, int coeff_type,
                                const VP8Matrix *const mtx, int lambda) {
...
static int ReconstructIntra16(VP8EncIterator *const it, VP8ModeScore *const rd,
...
static int ReconstructIntra4(VP8EncIterator *const it, int16_t levels[16],
...
static void SimpleQuantize(VP8EncIterator *const it, VP8ModeScore *const rd) {
  ...
  if (is_i16) {
    ...
    do {
      ...
      VP8MakeIntra4Preds(it);
      nz |= ReconstructIntra4(it, rd->y_ac_levels[it->i4_], src, dst, mode)
            << it->i4_;
...
static void RefineUsingDistortion(VP8EncIterator *const it, int try_both_modes,
                                  int refine_uv_mode, VP8ModeScore *const rd) {
  ...
  if (try_both_modes || !is_i16) {
    ...
    do {
      ...
      if (score_i4 >= best_score) {
        ...
        nz |= ReconstructIntra4(it, tmp_levels[it->i4_], src, tmp_dst,
                                best_i4_mode)
              << it->i4_;
      }
...
// if ((it->x_ >= 118 && it->y_ == 0) || (it->x_ == 0 && it->y_ == 1)) {
  ...
  // if ((it->x_ >= 118 && it->y_ == 0) || (it->x_ == 0 && it->y_ == 1)) {
  //   printf("line:%d[%d][%d]=====================================\n",
  //   __LINE__, it->y_, it->x_); PrintBlockInfo(it, rd);
  // }
  ...
  if (rd_opt > RD_OPT_NONE) {
    ...
    StopProfiling(&stop_watch, &timeVP8Decimate_BestIntra,
                  &countVP8Decimate_BestIntra);
  } else {
