...
static int TBufferNewPageFromKernel(VP8TBuffer *const b,
  ...
  static int i = 0;
  // printf("i:%d size:%d sizeof(*page):%d b->page_size_:%d
  // sizeof(token_t):%d\n",
  ...
  // printf("jj:%d page:%d b:%d b->pages:%d b->pages_->next_:%d b->last_page_:%d
  // *(b->last_page_):%d  b->left_:%d b->tokens_:%d sizeof(page->next_):%d
  // sizeof(unsigned long long):%d\n",
  //     j, page, b, b->pages_, b->pages_->next_, b->last_page_ ,
  //     *(b->last_page_), b->left_, b->tokens_, sizeof(page->next_),
  ...
  *b->last_page_ = page;
  ...
  // printf("j:%d page:%d b:%d b->pages:%d b->pages_->next_:%d b->last_page_:%d
  // *(b->last_page_):%d  b->left_:%d b->tokens_:%d sizeof(page->next_):%d
  // sizeof(unsigned long long):%d\n\n",
  //     j, page, b, b->pages_, b->pages_->next_, b->last_page_ ,
  //     *(b->last_page_), b->left_, b->tokens_, sizeof(page->next_),
  ...
  memcpy(b->tokens_,
         tokens_kernel->tokens_ + tokens_index * TOKENS_COUNT_PER_PAGE,
...
void ReadTokenFromKernel(VP8TBuffer *const b,
                         VP8TBufferKernel *const tokens_kernel) {
  int tokens_index = 0;
  for (tokens_index = 0; tokens_index < tokens_kernel->page_count_;
       tokens_index++) {
...
static int TBufferNewPage(VP8TBufferKernel *const b) {
  ...
  if (!b->error_) {
    ...
    printf("%s %s %d b->cur_page_:%d b->page_count_:%d\n", __FILE__,
           __FUNCTION__, __LINE__, b->cur_page_, b->page_count_);
  }
...
static WEBP_INLINE uint32_t AddToken(VP8TBufferKernel *const b, uint32_t bit,
                                     uint32_t proba_idx) {
  assert(proba_idx < FIXED_PROBA_BIT);
  assert(bit <= 1);
  if (b->left_ > 0 || TBufferNewPage(b)) {
    const int slot = --b->left_;
    b->tokens_[b->cur_page_ * TOKENS_COUNT_PER_PAGE + slot] =
        (bit << 15) | proba_idx;
  }
  return bit;
}
...
static WEBP_INLINE uint32_t AddToken(VP8TBuffer *const b, uint32_t bit,
  ...
  if (b->left_ > 0 || TBufferNewPage(b)) {
    ...
    //        __FUNCTION__, __LINE__, b->tokens_[slot], bit<<15, proba_idx, (bit
    //        << 15) | proba_idx);
...
void VP8TBufferInit(VP8TBuffer *const b) { (void)b; }
