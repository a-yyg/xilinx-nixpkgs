...
static void ApplyInverseTransforms(VP8LDecoder *const dec, int num_rows,
  ...
  StopProfiling(&stop_watch0, &timeApplyInverseTransforms,
                &countApplyInverseTransforms);
}
...
static int DecodeImageData(VP8LDecoder *const dec, uint32_t *const data,
  ...
  // fprintf(stderr, "width:%d height:%d NUM_ARGB_CACHE_ROWS:%d
...
static int ReadTransform(int *const xsize, int const *ysize,
  ...
  case CROSS_COLOR_TRANSFORM:
    ...
    ok = DecodeImageStream(
        ...
        VP8LSubSampleSize(transform->ysize_, transform->bits_), 0, dec,
        &transform->data_);
    break;
...
int VP8LDecodeAlphaImageStream(ALPHDecoder *const alph_dec, int last_row) {
  ...
  return alph_dec->use_8b_decode
             ? DecodeAlphaData(dec, (uint8_t *)dec->pixels_, dec->width_,
                               dec->height_, last_row)
