...
static void ParseIntraMode(VP8BitReader *const br, VP8Decoder *const dec,
  ...
  if (dec->use_skip_proba_)
    block->skip_ = VP8GetBit(br, dec->skip_p_);

  block->is_i4x4_ = !VP8GetBit(br, 145); // decide for B_PRED first
  if (!block->is_i4x4_) {
    // Hardcoded 16x16 intra-mode decision tree.
    const int ymode = VP8GetBit(br, 156)
...
int VP8ParseIntraModeRow(VP8BitReader *const br, VP8Decoder *const dec) {
  ...
  StopProfiling(&stop_watch, &timeVP8ParseIntraModeRow,
                &countVP8ParseIntraModeRow);
