...
static int RemapMemBuffer(WebPIDecoder *const idec, const uint8_t *const data,
                          size_t data_size) {
  MemBuffer *const mem = &idec->mem_;
...
WebPIDecoder *WebPINewDecoder(WebPDecBuffer *output_buffer) {
  ...
  idec->params_.output =
      (output_buffer != NULL) ? output_buffer : &idec->output_;
...
uint8_t *WebPIDecGetRGB(const WebPIDecoder *idec, int *last_y, int *width,
...
uint8_t *WebPIDecGetYUVA(const WebPIDecoder *idec, int *last_y, uint8_t **u,
                         uint8_t **v, uint8_t **a, int *width, int *height,
