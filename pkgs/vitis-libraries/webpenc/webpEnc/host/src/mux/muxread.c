...
static int MuxImageParse(const WebPChunk *const chunk, int copy_data,
  ...
  while (bytes != last) {
    ...
    case WEBP_CHUNK_ALPHA:
      ...
      if (ChunkSetNth(&subchunk, &wpi->alpha_, 1) != WEBP_MUX_OK)
        goto Fail;
      wpi->is_partial_ = 1; // Waiting for a VP8 chunk.
      break;
    case WEBP_CHUNK_IMAGE:
      if (ChunkSetNth(&subchunk, &wpi->img_, 1) != WEBP_MUX_OK)
        goto Fail;
    ...
    case WEBP_CHUNK_UNKNOWN:
      ...
      if (ChunkSetNth(&subchunk, &wpi->unknown_, 0) != WEBP_MUX_OK)
        goto Fail;
      break;
    default:
      goto Fail;
...
WebPMux *WebPMuxCreateInternal(const WebPData *bitstream, int copy_data,
  ...
  while (data != end) {
    ...
    case WEBP_CHUNK_ALPHA:
      ...
      if (ChunkSetNth(&chunk, &wpi->alpha_, 1) != WEBP_MUX_OK)
        goto Err;
      wpi->is_partial_ = 1; // Waiting for a VP8 chunk.
      break;
    case WEBP_CHUNK_IMAGE:
      if (ChunkSetNth(&chunk, &wpi->img_, 1) != WEBP_MUX_OK)
        goto Err;
      if (!MuxImageFinalize(wpi))
        goto Err;
      wpi->is_partial_ = 0; // wpi is completely filled.
    ...
    default: // A non-image chunk.
      ...
      if (ChunkSetNth(&chunk, chunk_list, 0) != WEBP_MUX_OK)
        goto Err;
...
static WebPMuxError SynthesizeBitstream(const WebPMuxImage *const wpi,
  ...
  const size_t size =
      RIFF_HEADER_SIZE + vp8x_size + alpha_size + ChunkDiskSize(wpi->img_);
...
static WebPMuxError MuxGetFrameFragmentInternal(const WebPMuxImage *const wpi,
  ...
  if (frame_frgm_data->size < kChunks[idx].size)
    return WEBP_MUX_BAD_DATA;
  // Extract info.
