...
static void WebPUtilClearPic(WebPPicture *const picture,
  ...
  if (rect != NULL) {
    ClearRectangle(picture, rect->x_offset_, rect->y_offset_, rect->width_,
                   rect->height_);
  } else {
...
WebPAnimEncoder *
WebPAnimEncoderNewInternal(int width, int height,
...
static WebPEncodingError GenerateCandidates(
  ...
  const int use_blending =
      !is_key_frame && IsBlendingPossible(prev_canvas, curr_canvas, rect);
  ...
  if (candidate_ll->evaluate_) {
    ...
    error_code =
        EncodeCandidate(sub_frame, rect, config_ll, use_blending, candidate_ll);
...
static void PickBestCandidate(WebPAnimEncoder *const enc,
  ...
  for (i = 0; i < CANDIDATE_COUNT; ++i) {
    if (candidates[i].evaluate_) {
      if (i == best_idx) {
        WebPMuxFrameInfo *const dst = is_key_frame ? &encoded_frame->key_frame_
...
static WebPEncodingError SetFrame(WebPAnimEncoder *const enc,
  ...
  *frame_skipped = 0;
  ...
  if (try_dispose_none) {
    error_code = GenerateCandidates(enc, candidates, WEBP_MUX_DISPOSE_NONE,
                                    is_lossless, is_key_frame, &rect_none,
...
static int FrameToFullCanvas(WebPAnimEncoder *const enc,
  ...
  if (enc->options_.allow_mixed) {
    if (!EncodeFrame(&enc->last_config_, canvas_buf, &mem2))
      goto Err;
