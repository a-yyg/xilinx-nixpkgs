...
ThreadInversePredictorRows(uint32_t *data, const VP8LTransform *const transform,
  ...
  for (y = y_start; y < y_end; y += ip_sync->num_workers) {
    ...
    const uint32_t *pred_mode_src =
        pred_mode_base + (y >> transform->bits_) * tiles_per_row;
    VP8LPredictorFunc pred_func;
    int x = 1;
    int t = 1;
    // .. the rest:
...
static int InversePredictorWorker(IpSync *const ip_sync,
  ...
  ThreadInversePredictorRows(ip_data->data, ip_data->transform, ip_data->start,
                             ip_data->stop, ip_data, ip_sync);
  return 1;
}
...
static void InversePredictorAlloc(IpSync *ip_sync, int rows, int width,
                                  int num_workers) {
  ip_sync->rows = rows;
#ifdef WEBP_USE_THREAD
  {
    int i;
    ip_sync->mutex_ = (pthread_mutex_t *)WebPSafeMalloc(
        (uint64_t)rows, sizeof(*ip_sync->mutex_));
    if (ip_sync->mutex_) {
      for (i = 0; i < rows; ++i) {
  ...
  ip_sync->cur_col =
      (int *)WebPSafeMalloc((uint64_t)rows, sizeof(*ip_sync->cur_col));
...
void VP8LAddGreenToBlueAndRed_C(uint32_t *data, int num_pixels) {
  ...
  StopProfiling(&stop_watch, &timeVP8LAddGreenToBlueAndRed,
                &countVP8LAddGreenToBlueAndRed);
}
...
static void ColorSpaceInverseTransform(const VP8LTransform *const transform,
  ...
  StopProfiling(&stop_watch, &timeColorSpaceInverseTransform,
                &countColorSpaceInverseTransform);
}
...
void VP8LInverseTransform(const VP8LTransform *const transform, int row_start,
                          ...
                          uint32_t *const out, int thread_number) {
  const int width = transform->xsize_;
  ...
  case PREDICTOR_TRANSFORM:
    ...
    StartProfiling(&stop_watch);
    if (thread_number != 0) {
      PredictorInverseTransformMt(transform, row_start, row_end, out,
                                  thread_number);
    } else {
      PredictorInverseTransform(transform, row_start, row_end, out);
    }

    StopProfiling(&stop_watch, &timePredictorInverseTransform,
                  &countPredictorInverseTransform);
  ...
  case COLOR_INDEXING_TRANSFORM:
    ...
    StopProfiling(&stop_watch2, &timeColorIndexInverseTransform,
                  &countColorIndexInverseTransform);
    break;
  }
}
...
void VP8LConvertBGRAToRGBA_C(const uint32_t *src, int num_pixels,
                             uint8_t *dst) {
  const uint32_t *const src_end = src + num_pixels;
  while (src < src_end) {
    const uint32_t argb = *src++;
...
WEBP_TSAN_IGNORE_FUNCTION void VP8LDspInit(void) {
  if (lossless_last_cpuinfo_used == VP8GetCPUInfo)
    return;
