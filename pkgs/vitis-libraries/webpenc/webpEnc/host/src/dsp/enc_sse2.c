...
static void FTransformPass1(const __m128i *const in01,
                            const __m128i *const in23, __m128i *const out01,
                            __m128i *const out32) {
...
static WEBP_INLINE void VerticalPred(uint8_t *dst, const uint8_t *top,
                                     int size) {
  if (top != NULL) {
    if (size == 8) {
      VE8uv(dst, top);
...
static WEBP_INLINE void HorizontalPred(uint8_t *dst, const uint8_t *left,
                                       int size) {
  if (left != NULL) {
    if (size == 8) {
      HE8uv(dst, left);
...
static WEBP_INLINE void DC8uvNoTopLeft(uint8_t *dst) { Put8x8uv(0x80, dst); }
...
static WEBP_INLINE void DC16NoTopLeft(uint8_t *dst) { Put16(0x80, dst); }
