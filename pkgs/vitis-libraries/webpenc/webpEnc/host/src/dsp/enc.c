...
static WEBP_INLINE int GetSSE(const uint8_t *a, const uint8_t *b, int w,
                              int h) {
  int count = 0;
  int y, x;
  for (y = 0; y < h; ++y) {
...
static void Copy4x4(const uint8_t *src, uint8_t *dst) { Copy(src, dst, 4, 4); }
...
WEBP_TSAN_IGNORE_FUNCTION void VP8EncDspInit(void) {
  if (enc_last_cpuinfo_used == VP8GetCPUInfo)
    return;
