...
static WEBP_INLINE void Load8x8x2(const uint8_t *const u,
                                  const uint8_t *const v, int stride,
                                  uint8x16_t *const p3, uint8x16_t *const p2,
                                  uint8x16_t *const p1, uint8x16_t *const p0,
                                  uint8x16_t *const q0, uint8x16_t *const q1,
                                  uint8x16_t *const q2, uint8x16_t *const q3) {
...
static WEBP_INLINE void Store2x8(const uint8x8x2_t v, uint8_t *const dst,
                                 int stride) {
...
#if !defined(WORK_AROUND_GCC)
static WEBP_INLINE void Store4x8(const uint8x8x4_t v, uint8_t *const dst,
                                 int stride) {
...
static void SimpleHFilter16(uint8_t *p, int stride, int thresh) {
  __asm__ volatile(
      ...
      LOAD8x4(d2, d3, d4, d5, [r4], [r5], r6) LOAD8x4(
          d24, d25, d26, d27, [r4], [r5],
          r6) "vswp       d3, d24                        \n" // p1:q1 p0:q3
              "vswp       d5, d26                        \n" // q0:q2 q1:q4
              "vswp       q2, q12                        \n" // p1:q1 p0:q2
                                                             // q0:q3 q1:q4
      ...
      DO_FILTER2(q1, q2, q12, q13, % [thresh])
          ...
          "vswp        d5, d24                       \n" STORE8x2(
              d4, d5, [%[p]], % [stride]) STORE8x2(d24, d25, [%[p]], % [stride])
...
static void ApplyFilter4(const int8x16_t p1, const int8x16_t p0,
                         const int8x16_t q0, const int8x16_t q1,
...
static void DoFilter4(const uint8x16_t p1, const uint8x16_t p0,
                      const uint8x16_t q0, const uint8x16_t q1,
...
static void ApplyFilter6(const int8x16_t p2, const int8x16_t p1,
...
static void DoFilter6(const uint8x16_t p2, const uint8x16_t p1,
                      const uint8x16_t p0, const uint8x16_t q0,
                      const uint8x16_t q1, const uint8x16_t q2,
                      const uint8x16_t mask, const uint8x16_t hev_mask,
  ...
  // do_filter6 part (complex loopfilter on pixels without hev)
  {
    ...
    const int8x16_t complex_lf_delta =
        vandq_s8(delta0, vreinterpretq_s8_u8(complex_lf_mask));
    ApplyFilter6(p2s, p1s, p0s, q0s, q1s, q2s, complex_lf_delta, op2, op1, op0,
                 oq0, oq1, oq2);
  }
}
...
static void VFilter16(uint8_t *p, int stride, int thresh, int ithresh,
  ...
  Load16x8(p, stride, &p3, &p2, &p1, &p0, &q0, &q1, &q2, &q3);
  {
    ...
    DoFilter6(p2, p1, p0, q0, q1, q2, mask, hev_mask, &op2, &op1, &op0, &oq0,
              &oq1, &oq2);
