...
#define MAP_COLOR_FUNCS(FUNC_NAME, TYPE, GET_INDEX, GET_VALUE)                 \
  static void FUNC_NAME(const TYPE *src, const uint32_t *const color_map,      \
...
static WEBP_INLINE uint32_t Select(uint32_t a, uint32_t b, uint32_t c) {
  int temp0, temp1, temp2, temp3, temp4, temp5;
  __asm__ volatile("cmpgdu.lt.qb %[temp1], %[c],     %[b]             \n\t"
                   "pick.qb      %[temp1], %[b],     %[c]             \n\t"
                   "pick.qb      %[temp2], %[c],     %[b]             \n\t"
                   ...
                   : [temp1] "=&r"(temp1), [temp2] "=&r"(temp2),
                     ...
                     [temp5] "=&r"(temp5), [temp0] "=&r"(temp0), [a] "+&r"(a)
                   : [b] "r"(b), [c] "r"(c));
  return a;
}
...
static void TransformColorInverse(const VP8LMultipliers *const m,
  ...
  if (num_pixels & 1)
    VP8LTransformColorInverse_C(m, data, 1);
}
...
static void ConvertBGRAToRGBA(const uint32_t *src, int num_pixels,
                              uint8_t *dst) {
  int temp0, temp1, temp2, temp3;
  const uint32_t *const p_loop1_end = src + (num_pixels & ~3);
  const uint32_t *const p_loop2_end = src + num_pixels;
...
static void ConvertBGRAToRGBA4444(const uint32_t *src, int num_pixels,
                                  uint8_t *dst) {
  int temp0, temp1, temp2, temp3, temp4, temp5;
  const uint32_t *const p_loop1_end = src + (num_pixels & ~3);
  const uint32_t *const p_loop2_end = src + num_pixels;
...
static void ConvertBGRAToRGB565(const uint32_t *src, int num_pixels,
                                uint8_t *dst) {
  int temp0, temp1, temp2, temp3, temp4, temp5;
  const uint32_t *const p_loop1_end = src + (num_pixels & ~3);
  const uint32_t *const p_loop2_end = src + num_pixels;
...
static void ConvertBGRAToBGR(const uint32_t *src, int num_pixels,
                             uint8_t *dst) {
  int temp0, temp1, temp2, temp3;
  const uint32_t *const p_loop1_end = src + (num_pixels & ~3);
  const uint32_t *const p_loop2_end = src + num_pixels;
