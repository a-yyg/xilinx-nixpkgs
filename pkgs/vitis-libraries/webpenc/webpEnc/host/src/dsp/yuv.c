...
WEBP_TSAN_IGNORE_FUNCTION void WebPInitSamplers(void) {
  if (yuv_last_cpuinfo_used == VP8GetCPUInfo)
    return;
...
void WebPConvertRGBA32ToUV_C(const uint16_t *rgb, uint8_t *u, uint8_t *v,
                             int width) {
  int i;
...
void (*WebPConvertRGBA32ToUV)(const uint16_t *rgb, uint8_t *u, uint8_t *v,
                              int width);
...
WEBP_TSAN_IGNORE_FUNCTION void WebPInitConvertARGBToYUV(void) {
  if (rgba_to_yuv_last_cpuinfo_used == VP8GetCPUInfo)
    return;
